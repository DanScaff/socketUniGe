...
/*** TO BE DONE START ***/
	...
	for (rp = pong_addrinfo; rp != NULL; rp = rp->ai_next) {
		udp_socket = socket(pong_addrinfo->ai_family, pong_addrinfo->ai_socktype, 0);
		if (udp_socket == -1)
			continue;
...
/*** TO BE DONE START ***/
	...
	for (rp = server_addrinfo; rp != NULL; rp = rp->ai_next) {
		server_socket = socket(server_addrinfo->ai_family, server_addrinfo->ai_socktype, 0);
		if (server_socket == -1)
			continue;
